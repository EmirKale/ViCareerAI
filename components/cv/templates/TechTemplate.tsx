...
export const TechTemplate = ({ data }: { data: CVData }) => (
    <Document title={`${data.personal?.fullName || 'CV'} - Technical Resume`}>
        <Page size="A4" style={styles.page}>
            ...
            <View style={styles.header}>
                ...
                <View style={styles.contactRow}>
                    {data.personal?.email ? <Text style={styles.contactItem}>[e] {data.personal.email}</Text> : null}
                    {data.personal?.phone ? <Text style={styles.contactItem}>[p] {data.personal.phone}</Text> : null}
                    {data.personal?.location ? <Text style={styles.contactItem}>[l] {data.personal.location}</Text> : null}
                </View>
                <View style={styles.contactRow}>
                    {data.personal?.linkedin ? <Text style={styles.contactItem}>[in] {data.personal.linkedin}</Text> : null}
                    {data.personal?.website ? <Text style={styles.contactItem}>[w] {data.personal.website}</Text> : null}
                </View>
            </View>
                ...
                <View style={styles.section}>
                    ...
                    {(data.projects || []).map((proj) => (
                        <View key={proj.id} style={styles.projectItem}>
                            ...
                            {proj.url ? <Text style={styles.projectUrl}>{proj.url}</Text> : null}
